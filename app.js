...
// Imprimir en la página HTML, mediante document.write la lista de consultas médicas de Dental. 

let dentales = dental.reduce(function (acumulador, dental) {
    return acumulador + dental.hora + ' - ' + dental.especialista + ' - ' + dental.paciente + ' - ' + dental.rut + ' - ' + dental.prevision + '</br>';
}, '');
document.write(dentales);

// Imprimir un listado total de todos los pacientes que se atendieron en el centro médico.
document.write('<h2>Listado pacientes radiología</h2>');
for (i = 0; i < radiologia.length; i++) {
    document.write(radiologia[i].paciente + '<br>');
}

document.write('<h2>Listado pacientes traumatología</h2>');
for (i = 0; i < traumatologia.length; i++) {
    document.write(traumatologia[i].paciente + '<br>');
}

document.write('<h2>Listado pacientes odontología</h2>');
for (i = 0; i < dental.length; i++) {
    document.write(dental[i].paciente + '<br>');
}

//Modificar mediante funciones las previsiones de Dental

document.write('<h2>Cambio de previsión en dental</h2>');


for (i = 0; i < dental.length; i++) {
    if (dental[i].prevision === 'ISAPRE') {
        (dental[i].prevision = 'FONASA');
    }
    else if (dental[i].prevision === 'FONASA') {
        (dental[i].prevision = 'ISAPRE');
    }
    document.write('<b>PACIENTE: </b>' + dental[i].paciente + '<b>| PREVISIÓN: </b>' + dental[i].prevision + '<b>| RUT: </b>' + dental[i].rut + '<br>');
}
